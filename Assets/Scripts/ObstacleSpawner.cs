...
//        {
//            // Stop or reflect movement on collision
//            speed = 0f;

//            // Optional bounce-back effect
//            Vector3 bounceDir = (transform.position - collision.transform.position).normalized;
//            transform.position += bounceDir * 0.5f;

//            // Optional: destroy obstacle after hitting blocker
...
public class ObstacleMovement : MonoBehaviour
{
    ...
    //        // Blocker paint is expected to have tag "Blocker"
    //        if (h.CompareTag("Blocker"))
    //        {
    //            hitBlocker = true;
    //        }

    //        // Other obstacles should have tag "Obstacle"
    //        if (h.CompareTag("obstacle"))
    //        {
    //            hitObstacle = true;
    //            // compute direction away from the other obstacle to separate slightly
    //            Vector2 away = (predictedPos - (Vector3)h.transform.position);
    //            float mag = away.magnitude;
    //            if (mag < 0.001f) away = Random.insideUnitCircle.normalized;
    //            else away /= mag;
    //            separation += away;
    //        }
    //    }

    //    // compute a timescale modifier to simulate resistance (slows progression along the motion curve)
    //    float timeScale = 1f;
    //    if (hitBlocker) timeScale *= blockerTimeScale;
    //    if (hitObstacle) timeScale *= obstacleTimeScale;

    //    // advance motion using scaled time — this preserves the same motion curves but slows them while "inside" blocker/near obstacles
    //    timeElapsed += Time.deltaTime * timeScale;

    //    // recompute final position for the (possibly reduced) timeElapsed
    //    Vector3 newPosition = CalculatePositionForTime(timeElapsed);

    //    // apply a small separation push (keeps obstacles from stacking on each other)
    //    if (separation != Vector2.zero)
    //    {
    //        separation.Normalize();
    //        newPosition += (Vector3)(separation * obstacleSeparationStrength);
    //    }

    //    transform.position = newPosition;

    //    // existing bonus / cleanup logic
    //    if (!bonusAwarded && player != null && player.transform.position.x > transform.position.x + 2f)
    //    {
    //        bonusAwarded = true;
    //        if (gameManager != null) gameManager.AddObstacleBonus();
    //    }
